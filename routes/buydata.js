...
const API_TOKEN = process.env.VTU_API_TOKEN; // ensure correct env

// ─────────────────────────────────────────────
// NETWORK MAP (CRITICAL FIX)
// ─────────────────────────────────────────────
const networkMap = {
  MTN: 1,
  GLO: 2,
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    console.log("🚀 EasyAccess Request:", requestPayload);
    ...
    try {
      ...
      return res.status(502).json({
        success: false,
        message: "API request failed. Wallet refunded.",
      });
    }
