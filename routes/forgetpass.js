...
router.post("/request-reset", async (req, res) => {
  ...
  try {
    ...
    await db`
      UPDATE users SET reset_code = ${code}, reset_expires = ${expires} WHERE id = ${user.id}
    `;
...
router.post("/verify-code", async (req, res) => {
  ...
  try {
    const users = await db`
      SELECT * FROM users WHERE (email = ${emailOrPhone} OR phone = ${emailOrPhone}) AND reset_code = ${code}
    `;
...
router.post("/reset-password", async (req, res) => {
  ...
  try {
    ...
    await db`
      UPDATE users
      SET password = ${hashed}, reset_code = NULL, reset_expires = NULL
      WHERE id = ${user.id}
    `;
